 ...
 * Algorithms" (12.4). Ported from osgHimmel's `siderealTime()`
 * (siderealtime.cpp).
 */
export function meanSiderealTime(time: AstronomicalTime): number {
  ...
  const t =
    280.46061837 +
    360.98564736629 * julianDaysSinceStandardEquinox(jd) +
...
/**
 * Faster mean sidereal time approximation, in degrees, from Jensen et
 * al., "A Physically-Based Night Sky Model" (2001). Ported from
 * osgHimmel's `siderealTime2()` (siderealtime.cpp).
 */
