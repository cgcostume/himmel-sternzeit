...
/**
 * A calendar date/time used for astronomical calculations, ported from
 * osgHimmel's `s_AstronomicalTime` (atime.h/.cpp).
 */
 ...
 * Algorithms" (ch. 7). Ported from osgHimmel's `jd()` (julianday.cpp).
 * Only valid for dates on/after the Gregorian calendar reform
 * (1582-10-15); returns 0 for the few days it skipped.
 */
export function julianDay(time: AstronomicalTime): JulianDay {
  ...
  return (
    toInt(365.25 * (year + 4716)) +
    toInt(30.600001 * (month + 1)) +
    day +
    h +
    b -
    1524.5
...
/**
 * Inverse of {@link julianDay}. Ported from osgHimmel's `makeTime()`
 * (julianday.cpp).
 */
export function fromJulianDay(
  jd: JulianDay,
  utcOffsetSeconds = 0,
): AstronomicalTime {
